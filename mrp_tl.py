...
if args.debug:
    print(f"<<<< DEBUG 0 >>>> project path from config.yaml: "
          f"{dlc.auxiliaryfunctions.read_plainconfig(config_path)['project_path']}")

if args.debug:
    print(f"<<<< DEBUG 1 >>>> project path from ITERATION-0 pose_cfg.yaml: "
          f"{dlc.auxiliaryfunctions.read_plainconfig(pose_config_path_it0)['project_path']}")
...
anno_file = Path("coco2017_all.h5") # TODO: change to coco2017_all.h5
...
if args.debug:
    print(f"<<<< DEBUG 2 >>>> project path from ITERATION-1 pose_cfg.yaml: "
          f"{dlc.auxiliaryfunctions.read_plainconfig(pose_config_path)['project_path']}")
...
if args.debug:
    print(f"<<<< DEBUG 3 >>>> project path from ITERATION-1 pose_cfg.yaml AFTER UPDATE: "
          f"{dlc.auxiliaryfunctions.read_plainconfig(pose_config_path)['project_path']}")
...
dlc.train_network(
    ...
    maxiters=10000,
    gputouse=0, # TODO: just try setting it to 0 or 1
)
